...
def select_source():
    ...
    source_path = filedialog.askopenfilename(
        filetypes=[
            ("AMS Source Files", "*.pdf;*.xlsx;*.xlsm"),
            ("PDF Files", "*.pdf"),
            ("Excel Files", "*.xlsx;*.xlsm"),
            ("All Files", "*.*"),
        ]
    )
    pdf_label.config(text=source_path)
...
def process():
    if not source_path or not excel_path:
    ...
    try:
        # ================= PDF EXTRACTION (CODE MATCH ONLY) =================
        data_by_code = {}
        ...
        def parse_table_like_rows(table_rows):
            ...
            for row in data_rows:
                ...
                # User requirement: Length (M) must come from the 5th column.
                # (1-based col 5 => 0-based index 4)
        ...
        for row in range(header_row + 1, ws.max_row + 1):
            ...
            for code in codes:
                ...
                if info and info.get("price") is not None:
                    total_rate += info["price"]
                    if info.get("length") is not None:
                        matched_lengths.append(info["length"])
                else:
                    print(f"Missing AMS code: {code}")

            if matched_lengths:
                # For multi-code cells, use the first matched AMS length.
                ams_length_cell.value = matched_lengths[0]
                ams_length_cell.fill = yellow_fill

            if total_rate > 0:
                ams_cost_cell.value = round(total_rate, 2)
        ...
        messagebox.showinfo("Done", f"Updated file saved:\n{output_file}")
...
# ================= GUI =================
root = tk.Tk()
root.title("AMS Price Updater")
root.geometry("500x300")

tk.Button(root, text="Select AMS Source (PDF/Excel)", command=select_source).pack(pady=5)
pdf_label = tk.Label(root, text="")
pdf_label.pack()

tk.Button(root, text="Select Excel", command=select_excel).pack(pady=5)
...
tk.Radiobutton(root, text="Class 1", variable=class_var, value="1").pack()
tk.Radiobutton(root, text="Class 2", variable=class_var, value="2").pack()

tk.Button(root, text="Update Prices", command=process).pack(pady=20)
